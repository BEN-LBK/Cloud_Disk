 ...
 *
 * 思路流程
 * 1、收到“/dealsharefile?cmd=xxxxx”指令,解析数据段中的cmd指令
 * 2、读取数据段信息，并提取用户名、MD5、文件名
 * 3、对比指令信息处理不同指令下的程序
 * 		1、save命令
 *		    a)先判断此文件是否已经分享存在，如果存在 直接返回已存在状态码
 *		    b)查看mysql中file_info表中该文件的count值并做+1更新
 *			c)user_file_list插入该文件信息
 *			d)更新user_file_count表中count值并做+1更新或者插入
 *		2、cancel命令
 *			a)登陆mysql数据库 将user_file_list该文件的分享状态置为0
 * 			b)更新或删除user_file_count中分享字段计数减一
 *			c)删除share_file_list表中的该文件信息
 *			d)从redis数据库共享列表中删除该成员
 *			e)从redis数据库共享名称hash中删除该成员
 *		3、pv命令
 *			a)查询该文件在mysql数据库上的下载量值 
 *			b)更新该文件在mysql数据库中的pv值 
 * 			c)插入或更新redis数据库中共享列表中该文件对应的score值+1
 * 			d)如果不存在则从redis数据库共享名称hash中插入该文件信息
 *	4、返回处理码			
 */
