 ...
 * @brief   上传文件后台CGI程序
 * 思路流程：
 *	1、收到"/upload"指令消息 先解析请求行中的指令
 *  2、从web中读取内容 提取文件的分界线、用户名、文件名、MD5码文件大小
 * 	3、提取文件内容并写入到本地保存为临时文件
 * 	4、创建父子进程 
 * 			子进程执行fdfs_file_upload 命名将本地保存的临时文件上传到后台分布式文件系统中
 * 			父进程读取上传成功后返回的文件路径
 * 	5、利用父子进程查询文件信息和后台存储服务器的IP等信息拼接成可直接访问的URL
 * 	6、将该文件的各种信息写入到MySQL数据库中，更新file_info表、user_file_list表、user_file_count表
 */
...
int upload_to_dstorage(char *filename, char *fileid)
{
    ...
    if(pid == 0) //子进程
    {
        ...
        //将标准输出 重定向 写管道
        dup2(fd[1], STDOUT_FILENO); //dup2(fd[1], 1); //在标准输出打印存放的路径名 通过写管道传给父进程


        //通过execlp执行fdfs_upload_file
        //execlp("fdfs_upload_file", "fdfs_upload_file", FDFS_CLIENT_CONF, filename, NULL); //在标准输出打印存放的路径名
