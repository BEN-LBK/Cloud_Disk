 ...
 *
 * 思路流程
 * 1、收到“/md5”指令,解析数据段中的json数据
 * 2、登陆redis数据库验证token，成功后在MySQL中查询数据库汇总是否有该文件的MD5
 *     如果没有 返回错误码  007 不能秒传
 *     如果有 
 *			查看该用户是否已存在该文件 如果存在则返回码 005 文件已存在 否则：
 * 			1、修改数据库file_info表中的count字段+1 
 * 			2、user_file_list 插入一条数据
 * 			3、返回成功码 006
 */
