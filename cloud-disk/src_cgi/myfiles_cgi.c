 ...
 * @brief  用户列表展示CGI程序
 * 思路流程：
 *	1、收到"/myfile?cmd=XXXX"指令消息 先解析请求行中的指令
 *	2、读取数据段的请求内容(用户名和token) 
 *	3、如果是获取文件数量则直接进入redis数据库验证token
 *	   	 进入MySQL数据库根据用户名找到所有文件并做统计 返回结果
 *	4、若是获取文件列表     进入redis数据库验证token 若验证错误 返回错误码
 *		 验证通过 进入MYSQL数据库 将指定数量的文件按指定顺序获取文件信息 并返回
 */
